...
use png::{self,Decoder,ColorType,BitDepth};
...
impl Image<RgbPixel> {
    fn pixelate(self, factor: usize) -> Result<Self,&'static str> {
        Ok(Image::<RgbPixel>::try_from(Image::<HsvPixel>::from(self).pixelate(factor)?)?)
    }
}
...
fn write_image(image: Image<RgbPixel>, path: &Path) -> Result<(),io::Error> {
...
fn main() {
    ...
    write_image(image.pixelate(16).expect("Pixelate failed."),Path::new(fileout)).expect("Writing png out failed.");
